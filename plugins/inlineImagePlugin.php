<?php
define('MY_WEB_ROOT',$_SERVER['DOCUMENT_ROOT']);
...
class inlineImagePlugin extends phplistPlugin
{
	...
	private function my_file_get_contents($src) {
		$str='';
		if (!$str = file_get_contents($src)){
			//if not full URL try to add web-root path to src value
			if(substr($src, 0, 1) != "/") {
				$src='/'.$src;
			}
			$src=$this->web_root.$src;
		}
	...
	*/
	function allowMessageToBeQueued($messagedata = array())
	{
		...
		if (preg_match_all('#<img[^<>]+\Winline(?:\W.*(?:/)?)?>#Ui', $msg, $match)) {
			...
			foreach ($match[0] as $val) {
				...
				if (!$str = $this->my_file_get_contents($src)){
	...
	private function make_full_uri($url,$domain,$scheme = 'http://'){
		$parse_url=parse_url($url);
		if(substr($parse_url['path'], 0, 1) != "/") {
			$parse_url['path']='/'.$parse_url['path'];
		}
		$url=parse_url($url, PHP_URL_HOST) === null ? $domain . $parse_url['path'] : $url;
		$url=parse_url($url, PHP_URL_SCHEME) === null ? $scheme . $url : $url;
		return $url;
	}

	private function try_replace($content,$original,$imagetag,$scheme = 'http://'){
		$content = str_replace($original, $imagetag, $content,$found);
		if ($found==0) {
			if (preg_match_all('#<img[^<>]+\Winline(?:\W.*(?:/)?)?>#Ui', $original, $match)) {
				foreach ($match[0] as $val) {
					$src = $this->getAttribute($val, "src");
					//try Full URI
					$domain=getConfig("domain");
					$uri=$this->make_full_uri($src,$domain,$scheme);
					$tmp_orig_new=str_replace($src, $uri, $original);
					$content = str_replace($tmp_orig_new, $imagetag, $content,$found1);
					//try https
					if ($found1==0){
						$content=$this->try_replace($content,$original,$imagetag,'https://');
					}
				}
			}
		}
	...
	*/
	...
	function messageQueued($id) {
		...
		foreach ($match[0] as $val) {
			$src = $this->getAttribute($val, "src");
    		$fcontents = $this->my_file_get_contents($src);
  	...
  	function parseOutgoingHTMLMessage($messageid, $content, $destination, $userdata = null) {
    	...
    	for ($i = 0; $i < $n; $i++) { // And replace them
  				//$content = str_replace($this->cache[$messageid][$i]['original'], $this->cache[$messageid][$i]['imagetag'], $content);
					//try to replace, while phplist makes the url to an uri
					$content=$this->try_replace($content,$this->cache[$messageid][$i]['original'],$this->cache[$messageid][$i]['imagetag']);
  		}
    	return $content;
    }
